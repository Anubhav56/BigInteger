...
class BigInteger{
	...
	public:
		...
		const string& getNumber(); //Retriving the number
		void setSign(bool s); //Setting sign
		const bool& getSign(); // Retriving the sign
		...
		BigInteger& operator ++(); //Prefix Increment overloaading
		...
		operator string(); // for converting from BigInteger to string
...
BigInteger BigInteger::operator -(BigInteger b){
	//Covering subtraction to addition
...
// Warning : Denominator must be written "long long" size not "BigInteger"(Limitation)
BigInteger BigInteger::operator /(BigInteger b){
...
// Warning : Denominator must be written "long long" size not "BigInteger"
BigInteger BigInteger:: operator %(BigInteger b){
	long long den = toInt(b.getNumber());
...
string BigInteger::subtract(string number1, string number2){
	...
	//Making both nnumber strings of equal length
...
string BigInteger::multiply(string n1, string n2){
	...
	//To assure n2 is greater than n1
	if(n1.length() > n2.length()) n1.swap(n2);
	string res = "0";
	
	//Mutiplying one by one digit of n1 to n2 and increasing one 0 at last each time
...
//Converting string to integer (upto long long range)
